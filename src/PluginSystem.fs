...
let registerPlugin (plugin: RegisteredPlugin) =
    ...
    if not (isCompatible plugin.Definition.Compatibility) then
        // 警告ログまたはエラー表示
        printfn "Warning: Plugin %s version %s may not be compatible with core version" 
            plugin.Definition.Name plugin.Definition.Version
    
    registeredPlugins <- registeredPlugins.Add(plugin.Definition.Id, plugin)
    
    // プラグインタブ情報の登録
    printfn "Plugin '%s' v%s registered with %d views, %d update handlers, %d command handlers, and %d tabs" 
...
let registerPluginFromJs (jsPlugin: obj) =
    try
        ...
        for key in updateKeys do
            let updateFn = updateHandlersObj?(key) |> unbox<obj -> obj -> obj>
            updateHandlers <- updateHandlers.Add(key, updateFn)
